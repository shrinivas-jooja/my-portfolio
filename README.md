# My Portfolio

## 👋 About Me

Electronics and embedded systems enthusiast with expertise in microcontroller programming, hardware development, and digital logic design. Currently focused on STM32 microcontrollers and Arduino platforms for innovative embedded solutions.

## 🛠️ Skills

### Microcontroller Platforms
- **STM32 Microcontrollers** - Advanced C/C++ programming, HAL/LL libraries, FreeRTOS
- **Arduino** - Prototyping and development with various Arduino boards
- **ARM Cortex-M** - Low-level hardware programming and optimization

### Programming Languages
- C/C++
- Python
- JavaScript

### Technologies & Tools
- Embedded Systems Design
- GPIO Configuration & Peripheral Management
- Digital Logic & Boolean Algebra
- Git Version Control
- Hardware Debugging Tools

## 📁 Projects

### Featured Projects

1. **SmartTAG – Smart Lost & Found System**
   - Developed a SmartTag system using dynamic QR codes for student item identification and a web app for verification.
   - Integrated SMS notifications to alert parents when lost items are found.
   - **Technologies**: Dynamic QR Code, Cloud Database, Web Application, SMS API.

2. **Memory Timeline AI – Personalized Learning Operating System (Ongoing)**
   - Developing an AI-powered personalized learning platform that enables users to build a searchable knowledge base.
   - Built a modular RAG architecture to support conversation memory, timeline-based learning, and context-aware retrieval.
   - **Technologies**: React, FastAPI, ChromaDB, Ollama/Gemini, Python.


3. **Smart Car Parking Management System**
   - Automated smart parking system using Arduino UNO, IR sensors, 20x4 LCD, and servo motor.
   - Real-time parking slot detection and automatic gate control.
   - **Technologies**: Arduino UNO, Embedded C, IR Sensors, 20×4 I2C LCD, MG995 Servo Motor.

📄 **View my full resume here**: [resume.md](./resume.md)

## 🔗 Connect With Me

- **GitHub**: [@shrinivas-jooja](https://github.com/shrinivas-jooja)
- **LinkedIn**: [shrinivas-jooja](https://linkedin.com/in/shrinivas-jooja)
- **Email**: [shrinivasjooja7@gmail.com](mailto:shrinivasjooja7@gmail.com)

## 📚 Learning & Development

- Advanced STM32 microcontroller applications & RTOS
- AI/ML models & Retrieval-Augmented Generation (RAG)
- Web development & cloud-database integration
- Hardware-software co-design


## 📊 Repository Statistics

![Profile Views](https://komarev.com/ghpvc/?username=shrinivas-jooja&color=blue)

---

**Last Updated**: December 2025

*Feel free to explore my repositories and reach out for collaborations!*
